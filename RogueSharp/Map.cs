...
namespace RogueSharp
{
   ...
   public class Map<TCell> : IMap<TCell> where TCell : ICell
   {
      ...
      private int ClampX( int x ) => Math.Max( 0, Math.Min( Width - 1, x ) );

      private int ClampY( int y ) => Math.Max( 0, Math.Min( Height - 1, y ) );
      ...
      public IEnumerable<TCell> GetCellsInCircle( int xCenter, int yCenter, int radius )
      {
         ...
         do
         {
            foreach ( TCell cell in GetCellsAlongLine( xCenter + x, yCenter + y, xCenter - x, yCenter + y ).Where( cell => AddToHashSet( discovered, cell ) ) )
            {
               yield return cell;
            }
            ...
            foreach ( TCell cell in GetCellsAlongLine( xCenter + y, yCenter + x, xCenter - y, yCenter + x ).Where( cell => AddToHashSet( discovered, cell ) ) )
            {
               yield return cell;
            }
